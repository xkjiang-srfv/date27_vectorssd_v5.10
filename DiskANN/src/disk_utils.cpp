                        ...
                        const std::string reorder_data_file)
{
    ...
    output_file_meta.push_back(medoid);                     // medoid
    output_file_meta.push_back(max_node_len);               // max_node_len
    output_file_meta.push_back(nnodes_per_sector);          // nnodes_per_sector
    output_file_meta.push_back(vamana_frozen_num);          // frozen_num
    output_file_meta.push_back(vamana_frozen_loc);          // frozen_loc
                     ...
                     const uint32_t Lf)
{
    ...
    {
        disk_pq_dims = atoi(param_list[5].c_str());     // param_list[5] = "0" ⇒ disk_pq_dims = 0
        use_disk_pq = true;
        if (disk_pq_dims == 0)
            use_disk_pq = false;                         
    }
    ...
    {
        if (1 == atoi(param_list[6].c_str()))           // param_list[6] = "0" ⇒ reorder_data = false
        {
            reorder_data = true;                         
        }
    }
    ...
    // final_index_ram_limit = B = 0.15
    double final_index_ram_limit = get_memory_budget(param_list[2]);
    if (final_index_ram_limit <= 0)
    {
        std::cerr << "Insufficient memory budget (or string was not in right format). Should be > 0." << std::endl;
        return -1;
    }

    // indexing_ram_budget = M = 40
    ...
    generate_quantized_data<T>(data_file_to_use, pq_pivots_path, pq_compressed_vectors_path, compareMetric, p_val,  num_pq_chunks, use_opq, codebook_prefix);
...
// This code should only be available in the "build" DLL.
    ...
    if (!use_disk_pq)
    {
        ...
        // diskann::create_disk_layout_raw_disk<T>(data_file_to_use.c_str(), mem_index_path, "/dev/nvme0n1", 0);
        // diskann::create_disk_layout_raw_disk<T>(data_file_to_use.c_str(), mem_index_path, "/dev/nvme0n1", std::string());
    }
    else
    {
        if (!reorder_data)
...
template <typename T>
void create_disk_layout_raw_disk(const std::string base_file, const std::string mem_index_file, const std::string output_file,const std::string reorder_data_file)
{
    uint32_t npts, ndims;
    std::cout<<"Treiiger RAW Device Write"<<std::endl;
    // amount to read or write in one shot
    size_t read_blk_size = 64 * 1024 * 1024;                 
    size_t write_blk_size = read_blk_size;
    cached_ifstream base_reader(base_file, read_blk_size);   
    ...
    diskann::cout << "Vamana index file size=" << actual_file_size << std::endl;
    std::ifstream vamana_reader(mem_index_file, std::ios::binary);   
    RawBlockWriter diskann_writer(output_file, defaults::SECTOR_LEN, /*use_direct=*/false);
    ...
    std::unique_ptr<char, decltype(&free)> sector_buf(
    (char*)aligned_alloc_or_throw(defaults::SECTOR_LEN, defaults::SECTOR_LEN), &free);                                 
    
    std::unique_ptr<char, decltype(&free)> multisector_buf(
    (char*)aligned_alloc_or_throw(defaults::SECTOR_LEN, ROUND_UP(max_node_len, defaults::SECTOR_LEN)), &free);

    std::unique_ptr<char, decltype(&free)> node_buf(
    (char*)aligned_alloc_or_throw(defaults::SECTOR_LEN, ROUND_UP(max_node_len, defaults::SECTOR_LEN)), &free);
    ...
    std::memset(sector_buf.get(), 0, defaults::SECTOR_LEN);
    diskann_writer.write(sector_buf.get(), defaults::SECTOR_LEN);
    ...
    std::unique_ptr<char, decltype(&free)> meta_sector(
    (char*)aligned_alloc_or_throw(defaults::SECTOR_LEN, defaults::SECTOR_LEN), &free);
    std::memset(meta_sector.get(), 0, defaults::SECTOR_LEN);

     
    std::memcpy(meta_sector.get(),
                output_file_meta.data(),
                output_file_meta.size() * sizeof(uint64_t));

     
    diskann_writer.write_at(0, meta_sector.get(), defaults::SECTOR_LEN);

    // sync + close
    diskann_writer.close_and_sync();
    diskann::cout << "Output disk index file written to " << output_file << std::endl;                   
}


template <typename T, typename LabelT>
int build_disk_index_with_nodeid(const char *dataFilePath, const char *indexFilePath, const char *indexBuildParameters,
                     diskann::Metric compareMetric, bool use_opq, const std::string &codebook_prefix, bool use_filters,
                     const std::string &label_file, const std::string &universal_label, const uint32_t filter_threshold,
                     const uint32_t Lf,const std::string& nvme_dev_path,uint32_t nsid)
{
    std::stringstream parser;                           
    parser << std::string(indexBuildParameters);        
    std::string cur_param;                              
    ...
    if (param_list.size() < 5 || param_list.size() > 9)  
    {
        diskann::cout << "Correct usage of parameters is R (max degree)\n L (indexing list size, better if >= R)\n B (RAM limit of final index in GB)\n M (memory limit while indexing)\n T (number of threads for indexing)\n B' (PQ bytes for disk index: optional parameter for very large dimensional data)\n reorder (set true to include full precision in data file: optional paramter, use only when using disk PQ\n build_PQ_byte (number of PQ bytes for inde build; set 0 to use full precision vectors)\n QD Quantized Dimension to overwrite the derived dim from B " << std::endl;
        return -1;
    }
    
     
    if (!std::is_same<T, float>::value && (compareMetric == diskann::Metric::INNER_PRODUCT || compareMetric == diskann::Metric::COSINE))
    {
        std::stringstream stream;
        stream << "Disk-index build currently only supports floating point data for Max Inner Product Search/ cosine similarity. "<< std::endl;
        throw diskann::ANNException(stream.str(), -1);
    }   
    #pragma endregion
    ...
    bool reorder_data = false;
    if (param_list.size() >= 7)                         // reorder_data = false                     
    {
        if (1 == atoi(param_list[6].c_str()))           // param_list[6] = "0" ⇒ reorder_data = false
        {
            reorder_data = true;                         
    ...
    if (param_list.size() >= 8)                         // build_pq_bytes = 0
    {
        build_pq_bytes = atoi(param_list[7].c_str());    
    }
    ...
    std::string disk_univ_label_file = disk_index_path + "_universal_label.txt";
    std::string disk_labels_int_map_file = disk_index_path + "_labels_map.txt";
    std::string dummy_remap_file = disk_index_path + "_dummy_map.txt"; // remap will be used if we break-up points of high label-density to create copies
    std::string sample_base_prefix = index_prefix_path + "_sample";
    // optional, used if disk index file must store pq data
    std::string disk_pq_pivots_path = index_prefix_path + "_disk.index_pq_pivots.bin";
    // optional, used if disk index must store pq data
    ...
    {
        Timer timer;
        std::cout << "Using Inner Product search, so need to pre-process base data into temp file. Please ensure there is additional (n*(d+1)*4) bytes for storing pre-processed base vectors, apart from the interim indices created by DiskANN and the final index." << std::endl;
        data_file_to_use = prepped_base;
        float max_norm_of_base = diskann::prepare_base_for_inner_products<T>(base_file, prepped_base);
        std::string norm_file = disk_index_path + "_max_base_norm.bin";
        diskann::save_bin<float>(norm_file, &max_norm_of_base, 1, 1);
    ...
    diskann::cout << "Starting index build: R=" << R << " L=" << L << " Query RAM budget: " << final_index_ram_limit << " Indexing ram budget: " << indexing_ram_budget << " T: " << num_threads << std::endl;
    auto s = std::chrono::high_resolution_clock::now();
    ...
    if (use_filters)
    {
        convert_labels_string_to_int(labels_file_original, labels_file_to_use, disk_labels_int_map_file,universal_label);
        augmented_data_file = index_prefix_path + "_augmented_data.bin";
        augmented_labels_file = index_prefix_path + "_augmented_labels.txt";
        if (filter_threshold != 0)
        {
            breakup_dense_points<T>(data_file_to_use, labels_file_to_use, filter_threshold, augmented_data_file, augmented_labels_file,dummy_remap_file); // RKNOTE: This has large memory footprint need to make this streaming
            data_file_to_use = augmented_data_file;
            labels_file_to_use = augmented_labels_file;
        }
    }
    ...
    if (use_disk_pq)
    {
        generate_disk_quantized_data<T>(data_file_to_use, disk_pq_pivots_path, disk_pq_compressed_vectors_path, compareMetric, p_val, disk_pq_dims);
    }
    #pragma endregion
    ...
    if (!use_disk_pq)
    {
        diskann::create_nodeid_disk_layout<T>(data_file_to_use, mem_index_path, disk_index_path + ".meta", nvme_dev_path, nsid, "");
    }
    else
    {
        if (!reorder_data)
            diskann::create_nodeid_disk_layout<uint8_t>(disk_pq_compressed_vectors_path,mem_index_path,disk_index_path + ".meta",nvme_dev_path,nsid,"");
        else
            diskann::create_nodeid_disk_layout<uint8_t>(disk_pq_compressed_vectors_path,mem_index_path,disk_index_path + ".meta",nvme_dev_path,nsid,data_file_to_use);
    }
    diskann::cout << timer.elapsed_seconds_for_step("generating disk layout") << std::endl;

     
    double ten_percent_points = std::ceil(points_num * 0.1);
    double num_sample_points = ten_percent_points > MAX_SAMPLE_POINTS_FOR_WARMUP ? MAX_SAMPLE_POINTS_FOR_WARMUP : ten_percent_points;
    double sample_sampling_rate = num_sample_points / points_num;
...
template DISKANN_DLLEXPORT int build_disk_index_with_nodeid<int8_t, uint32_t>(
    const char*, const char*, const char*, Metric, bool,
    const std::string&, bool, const std::string&, const std::string&,
    uint32_t, uint32_t, const std::string&, uint32_t);

template DISKANN_DLLEXPORT int build_disk_index_with_nodeid<uint8_t, uint32_t>(
    const char*, const char*, const char*, Metric, bool,
    const std::string&, bool, const std::string&, const std::string&,
    uint32_t, uint32_t, const std::string&, uint32_t);

template DISKANN_DLLEXPORT int build_disk_index_with_nodeid<float, uint32_t>(
    const char*, const char*, const char*, Metric, bool,
    const std::string&, bool, const std::string&, const std::string&,
    uint32_t, uint32_t, const std::string&, uint32_t);
