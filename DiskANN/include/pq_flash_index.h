...
{
  public:
    DISKANN_DLLEXPORT PQFlashIndex(std::shared_ptr<AlignedFileReader> &fileReader,diskann::Metric metric = diskann::Metric::L2);
    ...
    DISKANN_DLLEXPORT void set_nodeid_layout_params(bool use_nodeid_layout,
                              const std::string &nvme_dev_path,
                              uint32_t nsid)
        {
            _use_nodeid_layout = use_nodeid_layout;
            _nvme_dev_path = nvme_dev_path;
            _nvme_nsid = nsid;
        }
    ...
    DISKANN_DLLEXPORT std::vector<bool> read_nodes_nodeid(
                                        NodeIdNvmeWriter* nodeid_reader,
                                        const std::vector<uint32_t> &node_ids,
                                        std::vector<T *> &coord_buffers,
                                        std::vector<std::pair<uint32_t, uint32_t *>> &nbr_buffers);
    ...
    //
    std::vector<bool> read_nodes_file(
        const std::vector<uint32_t> &node_ids,
        std::vector<T *> &coord_buffers,
        std::vector<std::pair<uint32_t, uint32_t *>> &nbr_buffers);
  ...
  private:
    ...
    DISKANN_DLLEXPORT inline uint32_t get_nodeid_physical_id(uint32_t node_id) const
    {
        return static_cast<uint32_t>(_data_nodeid_base + node_id);
    }

    DISKANN_DLLEXPORT inline uint32_t get_reorder_physical_id(uint32_t node_id) const
    {
        return static_cast<uint32_t>(_reorder_nodeid_base + node_id);
    }
    ...
    bool _use_nodeid_layout = false;
    std::string _nvme_dev_path = "";
    uint32_t _nvme_nsid = 1;
    uint64_t _node_vec_bytes = 0;
    uint64_t _data_nodeid_base = 1;
    uint64_t _node_nblks = 0;
    uint64_t _reorder_nodeid_base = 0;
    uint64_t _reorder_nblks = 0;

    std::unique_ptr<diskann::NodeIdNvmeWriter> _nodeid_reader = nullptr;
