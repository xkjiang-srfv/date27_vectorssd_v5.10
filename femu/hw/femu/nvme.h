...
enum NvmeIoCommands {
    ...
    /*-specific NVMe IO commands (0x80-0xFF)*/
    NVME_CMD_NODEID_WRITE       = 0x81,
    NVME_CMD_NODEID_READ        = 0x82,
    NVME_CMD_AGGRE_READ         = 0x83,
...
typedef struct NvmeNodeCmd {
    uint8_t     opcode;     // 0x81 NodeID Write / 0x82 NodeID Read
    uint8_t     flags;
    uint16_t    cid;
    uint32_t    nsid;
...
typedef struct NvmeAggreReadCmd {
    uint8_t  opcode;      // 0x83
    uint8_t  flags;
    uint16_t cid;
    uint32_t nsid;
    ...
    uint64_t rsvd2;
    uint64_t mptr;        // ParamEntry host addr

    uint64_t prp1;        // Result buffer first page
    uint64_t prp2;        // Result second page or PRP list

    uint32_t result_nlb;  // result 4KB pages
    uint32_t slot_id;     // optional, can keep for debug
...
static inline void nvme_check_size(void)
{
    ...
    /*-specific NVMe IO commands (0x80-0x81)*/
    QEMU_BUILD_BUG_ON(sizeof(NvmeNodeCmd ) != 64);

     
    QEMU_BUILD_BUG_ON(sizeof(NvmeAggreReadCmd) != 64);
...
extern FILE *femu_log_fp;
extern FILE *femu_err_fp;
extern FILE *femu_debug_fp;

 
 

extern volatile sig_atomic_t femu_logging_enabled;
extern volatile sig_atomic_t femu_debug_enabled;

 

void femu_log_init(const char *log_dir);
void femu_log_signal_init(void);
void femu_log_close(void);

 

void femu_log_write(FILE *fp, const char *tag,
                    const char *fmt, ...);
...
#define femu_log(fmt, ...) \
    do { \
        if (femu_logging_enabled && femu_log_fp) { \
            femu_log_write(femu_log_fp, "LOG", fmt, ##__VA_ARGS__); \
        } \
    } while (0)
...
#define femu_err(fmt, ...) \
    do { \
        if (femu_err_fp) { \
            femu_log_write(femu_err_fp, "ERR", fmt, ##__VA_ARGS__); \
        } \
...
#define femu_debug(fmt, ...) \
    do { \
        if (femu_debug_enabled && femu_debug_fp) { \
            femu_log_write(femu_debug_fp, "DBG", fmt, ##__VA_ARGS__); \
        } \
    } while (0)
#else
