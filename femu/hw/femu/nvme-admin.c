...
static void nvme_init_poller(FemuCtrl *n)
{
    ...
    n->to_raw = g_malloc0(sizeof(struct rte_ring *) * (n->nr_pollers + 1));
    for (i = 1; i <= n->nr_pollers; i++) {
        n->to_raw[i] = femu_ring_create(FEMU_RING_TYPE_MP_SC, FEMU_MAX_INF_REQS);
        if (!n->to_raw[i]) {
            femu_err("failed to create ring (n->to_raw) ...\n");
            abort();
        }
        assert(rte_ring_empty(n->to_raw[i]));
    }
    ...
    for (i = 1; i <= n->nr_pollers; i++) {
        args[i].n = n;
        args[i].index = i;
        qemu_thread_create(&n->poller[i], "femu-nvme-poller", nvme_poller, &args[i], QEMU_THREAD_JOINABLE);
