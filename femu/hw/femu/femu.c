...
FILE *femu_log_fp   = NULL;
FILE *femu_err_fp   = NULL;
FILE *femu_debug_fp = NULL;

volatile sig_atomic_t femu_logging_enabled = 0;
volatile sig_atomic_t femu_debug_enabled   = 0;
...
static void femu_log_signal_handler(int sig)
{
    if (sig == SIGUSR1) {
        femu_logging_enabled = 1;
    } else if (sig == SIGUSR2) {
        femu_debug_enabled = 1;
    }
}
...
void femu_log_signal_init(void)
{
    ...
    sa.sa_handler = femu_log_signal_handler;
    sa.sa_flags   = SA_RESTART;

    sigaction(SIGUSR1, &sa, NULL);  /* enable femu_log */
    sigaction(SIGUSR2, &sa, NULL);  /* enable femu_debug */

    fprintf(stderr,
        "[FEMU] log signal control ready (PID=%d)\n"
        "[FEMU]   SIGUSR1 -> enable femu_log\n"
        "[FEMU]   SIGUSR2 -> enable femu_debug\n",
        getpid());
}
...
void femu_log_init(const char *dir)
{
    char path[256];

    mkdir(dir, 0755);

    snprintf(path, sizeof(path), "%s/femu.log", dir);
    femu_log_fp = fopen(path, "w");

    snprintf(path, sizeof(path), "%s/femu_err.log", dir);
    femu_err_fp = fopen(path, "w");

    snprintf(path, sizeof(path), "%s/femu_debug.log", dir);
    femu_debug_fp = fopen(path, "w");

    fprintf(stderr, "[FEMU] log files opened under %s\n", dir);
}
...
static void femu_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    static bool femu_log_inited = false;
    if (!femu_log_inited) {
        femu_log_init("/home/embed/Codes/vectorssd/femu/build-femu");
        femu_log_signal_init();
        femu_log_inited = true;

        fprintf(stderr,
            "[FEMU] log system initialized in femu_realize(), PID=%d\n",
            getpid());
    }
