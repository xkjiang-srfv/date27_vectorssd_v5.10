...
// #define LEAFTL_VERIFY

#ifdef LEAFTL_VERIFY
#define lea_verify(fmt, ...) \
    femu_log("[LEA-VERIFY] " fmt, ##__VA_ARGS__)
#else
#define lea_verify(fmt, ...)
#endif
...
static void invalidate_cmt_entry(struct ssd *ssd, uint64_t tvpn)
{
    struct cmt_mgmt *cm = &ssd->cm;
    struct hash_table *ht = &cm->ht;

    struct cmt_entry *cmt_entry = find_hash_entry(ht, tvpn);

    /* The translation page is not cached. */
    if (cmt_entry == NULL) {
        return;
    }

    lea_verify(
        "CMT_INVALIDATE tvpn=%"PRIu64
        " old_inter=%"PRIu64"\n",
        tvpn,
        cmt_entry->sm.inter
    );
    ...
    if (!delete_cmt_hashnode(ht, cmt_entry)) {
        ftl_err("failed to remove CMT entry for tvpn=%"PRIu64"\n",
                tvpn);
        return;
    }

    /* Remove it from the CMT LRU list. */
    QTAILQ_REMOVE(&cm->cmt_entry_list, cmt_entry, entry);

    /* Reset the entry and return it to the free list. */
    cmt_entry->dirty = CLEAN;
    cmt_entry->lpn = INVALID_LPN;
    cmt_entry->next = NULL;

    QTAILQ_INSERT_TAIL(
        &cm->free_cmt_entry_list,
        cmt_entry,
        entry
    );
    ...
    ftl_assert(cm->used_cmt_entry_cnt >= 0);
    ftl_assert(cm->free_cmt_entry_cnt <= cm->tt_entries);
}
...
static segment_param* segment_param_init(struct ssd *ssd) {
    ...
    sp->page_total_cnt = ssd->sp.tt_pgs;    
    sp->threshold = 10;              
...
static void regression(simple_segment* sg, uint64_t* lpns, uint64_t* ppns, int len, learned_mgmt* lm, int seg_no) {
    ...
    ftl_assert(sg != NULL);
    ftl_assert(lpns != NULL);
    ftl_assert(ppns != NULL);
    ftl_assert(lm != NULL);
    ftl_assert(lm->sp != NULL);
    ftl_assert(len > 0);
    ftl_assert(len <= TRAIN_LEN);
    ftl_assert(seg_no >= 0);
    ftl_assert(seg_no < lm->sp->sgement_cnt);
    ...
    uint64_t group_start = (uint64_t)seg_no * (uint64_t)lm->sp->mapping_per_page;

    ftl_assert(start_lpn / lm->sp->mapping_per_page ==(uint64_t)seg_no);
    ftl_assert(start_lpn >= group_start);

    segment_total_cnt++;

    int accurate = 1;
    for (int i = 0; i < len; i++) {
        ftl_assert(lpns[i] >= start_lpn);
        ftl_assert(ppns[i] >= start_ppn);

        off_x[i] = lpns[i]-start_lpn;
        off_y[i] = ppns[i]-start_ppn;
        if (i > 0) {
            if (off_x[i] - off_x[i-1] != 1  || off_y[i] - off_y[i - 1] != 1 )
                accurate = 0;
        }
    }
    ...
    if (!sg->is_accurate) {
        ...
        int pointer = 0;
        for (int i = 0; i < len; i++) {
            ftl_assert(lpns[i] < (uint64_t)lm->sp->page_total_cnt);
        ...
        // begin
        lea_verify(
            "CRB_CREATE seg_no=%d entries=%d start_lpn=%"PRIu64"\n",
            seg_no,
            tmp_bm->cnt,
            start_lpn
        );
    ...
    sg->k = k;                           
    sg->len = lpns[len-1] - lpns[0];     
    sg->slpa = lpns[0]-group_start;      
    sg->inter = b + start_ppn;           

    // begin
    lea_verify(
        "MODEL seg_no=%d start_lpn=%"PRIu64
        " slpa=%"PRIu64
        " len=%"PRIu64
        " accurate=%d"
        " k=%.6f"
        ...
        *ppn = off_lpn*sg->k + sg->inter;
        ...
        if (flag) {
            ...
            *ppn = crb_sm->k * off_lpn + crb_sm->inter;
            ...
            // copy simple_segment information back
            lru_en->k = crb_sm->k;
            lru_en->inter = crb_sm->inter;
            lru_en->len = crb_sm->len;
            lru_en->slpa = crb_sm->slpa;
            lru_en->is_accurate = crb_sm->is_accurate;
            lru_en->next = crb_sm->next;
        }
    }
...
static segment* segment_insert_simple_segment(
    segment* sg,
    simple_segment* new_seg,
    int len)
{
    ...
    /* new segment is completely before the head */
    if (end_lpn < sg_lb) {
    ...
    /*
     * new segment overlaps with the head.
     * It must be placed into a newer version/layer.
     */
    if (start_lpn <= sg_ub) {
        return sg;
    }
...
static int level_seg_mgmt_insert(level_seg_mgmt* lsegm, uint64_t* lpns, uint64_t* ppns, int len, segment_param* sp, learned_mgmt* lm) {
    ...
    ftl_assert(seg_no < (uint64_t)sp->sgement_cnt);
...
static uint64_t all_seg_get_ppn(
    ...
    simple_segment* lru_en)
{
    ...
    while (sg) {
        ...
        lea_verify(
            ...
            seg_no,
            sg->segment_no,
            sg->segment_head ? sg->segment_head->inter : INVALID_VAL,
            sg->segment_head ? sg->segment_head->k : 0.0,
            sg->segment_head ? sg->segment_head->slpa : INVALID_VAL,
            sg->segment_head ? sg->segment_head->len : -1
        );

        flag = level_seg_mgmt_search(
            sg,
            lpn,
            &ppn,
            sp,
    ...
    while (sm) {                                     
        if (sm->is_accurate) {                       
            ...
            if (crb->crb_unit[seg_no]) {             
                ...
                while (tmp && tmp->to_sm != sm) {           
                    tmp = tmp->next;
                }

                if(tmp){
                    for (int i = 0; i < tmp->cnt; i++) {     
                        if (tmp->record[i] >= slpa && tmp->record[i] - slpa < 256)     
...
static void delete_seg(learned_mgmt* lm, segment_param* sp, segment* seg, int seg_no, int* delete_cnt) {
    ...
    while (sm) {                                     
        ...
        if (!tmp->is_accurate) {                      
            cr_bm *cur = crb->crb_unit[seg_no];    
            cr_bm *prev = NULL;
            
            while(cur && cur->to_sm != tmp){
                prev = cur;
                cur = cur->next;
            }
            ...
            while (valid_seg) {                                          
                ...
                if (delete_flag) {
                    ...
                    segment* tmp = valid_seg;
                     lea_verify(
                        "COMPACT_DELETE seg_no=%d delete_cnt_before=%d\n",
                        i,
                        delete_cnt
                    );
...
// static uint64_t test_flush_cnt = 0;
static uint64_t buffer_flush(struct ssd *ssd, NvmeRequest *req) {
    ...
    bool *gtd_to_update = g_malloc0(sizeof(bool) * gtd_cnt);             
    if (gtd_to_update == NULL) {
        ftl_err("buffer_flush: failed to allocate gtd_to_update\n");
        return 0;
    }


     
    quick_sort(ssd->buffer_lpns, 0, sp->buffer_size-1);      
    ...
    for (int i = 0; i < buffer_size; i++) {                  
        ...
        uint64_t tvpn = lpns[i] / sp->ents_per_pg;           
        ftl_assert(tvpn < (uint64_t)gtd_cnt);
        gtd_to_update[tvpn] = true;                         
    }


     
    for (uint64_t tvpn = 0; tvpn < (uint64_t)gtd_cnt; tvpn++) {  
        if (!gtd_to_update[tvpn]) {
            continue;
        }
    ...
    for (int i = 0; i < buffer_size; i++) {
        ...
        if (req == NULL) {
            ...
            gcr.cmd = NAND_WRITE;
            gcr.stime = 0;
            sublat = ssd_advance_status(ssd, &wppa[i], &gcr);
    ...
    for (int i = 1; i < buffer_size; i++) {
        ...
        if (lpns[i] - lpns[i - 1] > (uint64_t)threshold) {       
            need_split = true;
        }

        if (i - segment_start >= TRAIN_LEN) {                    
            need_split = true;
        }

        if (lpns[i] / lm->sp->mapping_per_page != lpns[i - 1] / lm->sp->mapping_per_page) {   
            need_split = true;
        }

        if(need_split)
        {
            int len = i - segment_start;            
            ftl_assert(len > 0);
            ftl_assert(len <= TRAIN_LEN);
            
             
            flush_lpn_ppn_mappings(     
                lm,
                lpns + segment_start,   
                vppn + segment_start,
                len
    ...
    if (segment_start < buffer_size) {
        int len = buffer_size - segment_start;
        ftl_assert(len > 0);
        ftl_assert(len <= TRAIN_LEN);
        flush_lpn_ppn_mappings(
            lm,
    ...
    for (uint64_t tvpn = 0; tvpn < (uint64_t)gtd_cnt; tvpn++) {
    if (gtd_to_update[tvpn]) {
            invalidate_cmt_entry(ssd, tvpn);
        }
    }

    g_free(gtd_to_update);

    // /* Test 6 only: trigger compaction after base write + overwrite */
    // test_flush_cnt++;

    // lea_verify(
    //     "TEST6_FLUSH flush_cnt=%"PRIu64"\n",
    //     test_flush_cnt
    // );

    // if (test_flush_cnt == 8) {
    //     lea_verify(
    //         "COMPACT_TRIGGER flush_cnt=%"PRIu64"\n",
    //         test_flush_cnt
    //     );
...
LBA -> LPN
 ...
 CMT lookup (TP cache)
      |
      |
 learned segment search
      |
      +----------------+
      |                |
   model hit        model miss
      |                |
 predict vPPN       TP read
      |                |
      +-------+--------+
              |
        vPPN -> PPA
...
static uint64_t ssd_read(struct ssd *ssd, NvmeRequest *req)
{
    ...
    for (lpn = start_lpn; lpn <= end_lpn; lpn++) {       
        ...
        for (int i = 0; i < ssd->buffer_pointer; i++) {      
        ...
        // end
            ...
            int model_found = segment_search_simple_segment(&cmt_ent->sm, lpn, &vppn, ssd->lem, &lru_en);   
            ...
            else if (!is_same_segment(&lru_en, &cmt_ent->sm)) {
                cmt_ent->sm.inter = lru_en.inter;
                cmt_ent->sm.k = lru_en.k;
                cmt_ent->sm.len = lru_en.len;
        ...
        // Leaftl correctness verification begin : check if the predicted vppn is equal to the true vppn
        uint64_t true_vppn = INVALID_VAL;
        if (valid_ppa(ssd, &trueppa) && mapped_ppa(&trueppa)) {
            true_vppn = ppa2vppn(ssd, &trueppa);
        }
        lea_verify(
            "READ_MAP lpn=%"PRIu64" pred=%"PRIu64" true=%"PRIu64" result=%s\n",
            lpn,
            vppn,
            true_vppn,
            (vppn == true_vppn) ? "MATCH" : "MISPRED"
        );
        // Leaftl correctness verification end
        ...
        if (valid_ppa(ssd, &trueppa) && mapped_ppa(&trueppa)) {
            ...
            if (ppa2vppn(ssd, &trueppa) != vppn) {
                // Leaftl correctness verification begin : check if the predicted vppn is equal to the true vppn
                lea_verify(
                    "MISPRED lpn=%"PRIu64" pred=%"PRIu64" true=%"PRIu64"\n",
                    lpn,
                    vppn,
                    ppa2vppn(ssd, &trueppa)
                );

                lea_verify(
                    "WRONG_PAGE_READ lpn=%"PRIu64" vppn=%"PRIu64"\n",
                    lpn,
                    vppn
                );
            ...
            lea_verify(
                    "TRUE_PAGE_READ lpn=%"PRIu64" vppn=%"PRIu64"\n",
                    lpn,
                    ppa2vppn(ssd, &trueppa)
                );
...
static uint64_t ssd_write(struct ssd *ssd, NvmeRequest *req)
{
    ...
    for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
        ...
        if (!update_in_buffer && ssd->lem->bitmap[lpn]) {
            ppa = get_maptbl_ent(ssd, lpn);
            
             
            if (mapped_ppa(&ppa) && valid_ppa(ssd, &ppa) && get_pg(ssd, &ppa)->status == PG_VALID) {
                
                // begin
                lea_verify(
                    "OVERWRITE_OLD lpn=%"PRIu64" old_vppn=%"PRIu64"\n",
                    lpn,
                    ppa2vppn(ssd, &ppa)
                );
                // end
        ...
        // if (st->write_cnt % 1024 == 0) {
        //     lea_verify(
        //         "WRITE_CNT write_cnt=%"PRIu64"\n",
        //         st->write_cnt
        //     );
        // }
        ...
        if (st->write_cnt % 1000000 == 0) {
            lea_verify(
                "COMPACT_TRIGGER write_cnt=%"PRIu64"\n",
                st->write_cnt
            );
            compact(ssd, ssd->lem);
        }
