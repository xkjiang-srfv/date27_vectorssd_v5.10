...
// #define DFTL_VERIFY
// #define DFTL_FORCE_GC_TEST

#ifdef DFTL_VERIFY
#define dftl_verify(fmt, ...) \
    femu_log("[DFTL-VERIFY] " fmt, ##__VA_ARGS__)
#else
#define dftl_verify(fmt, ...)
#endif
...
static void ssd_init_params(struct ssdparams *spp, FemuCtrl *n)
{
    ...
    spp->ents_per_pg = spp->secs_per_pg * spp->secsz / sizeof(uint64_t);                                     
    spp->tt_gtd_size = spp->tt_pgs / spp->ents_per_pg;           
     


    spp->tt_cmt_size = CMT_SIZE;  // 100%     
...
static void evict_entry_from_cmt(struct ssd *ssd)
{
    ...
    dftl_verify(
        "CMT_EVICT lpn=%"PRIu64" ppn=%"PRIu64" dirty=%d\n",
        cmt_entry->lpn,
        cmt_entry->ppn,
        cmt_entry->dirty
    );
    ...
    if (cmt_entry->dirty == DIRTY) {
        ...
        if (!mapped_ppa(&ppa) || !valid_ppa(ssd, &ppa)) {
            dftl_verify(
                "TP_NEW_WRITE lpn=%"PRIu64" tvpn=%"PRIu64"\n",
                cmt_entry->lpn,
                tvpn
            );
            ...
            dftl_verify(
                "TP_REWRITE lpn=%"PRIu64" tvpn=%"PRIu64" old_ppn=%"PRIu64"\n",
                cmt_entry->lpn,
                tvpn,
                ppa2pgidx(ssd, &ppa)
            );
...
static struct nand_lun *process_translation_page_read(struct ssd *ssd, NvmeRequest *req, uint64_t lpn)
{
    ...
    dftl_verify(
        "TP_READ lpn=%"PRIu64" tvpn=%"PRIu64" ppn=%"PRIu64"\n",
        lpn,
        tvpn,
        ppa2pgidx(ssd, &ppa)
    );
...
static uint64_t gc_write_page(struct ssd *ssd, struct ppa *old_ppa)
{
    ...
    dftl_verify(
        "GC_DATA_MOVE lpn=%"PRIu64" old_ppn=%"PRIu64" new_ppn=%"PRIu64"\n",
        lpn,
        ppa2pgidx(ssd, old_ppa),
        ppa2pgidx(ssd, &new_ppa)
    );
    /* update maptbl */
...
static void clean_one_data_block(struct ssd *ssd, struct ppa *ppa)
{
    ...
    struct cmt_entry *cmt_entry;
    struct ppa new_ppa;
    uint64_t *batch_update = g_new(uint64_t, spp->pgs_per_blk);   
    int pos = 0, flag;
...
static int do_gc(struct ssd *ssd, bool force)
{
    ...
    dftl_verify(
        "GC_BEGIN line=%d type=%d ipc=%d vpc=%d free=%d\n",
        victim_line->id,
        victim_line->type,
        victim_line->ipc,
        victim_line->vpc,
...
static uint64_t ssd_read(struct ssd *ssd, NvmeRequest *req)
{
    ...
    for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
        ...
        cmt_entry *entry = cmt_hit(ssd, lpn);

        if (entry) {
            dftl_verify(
                "CMT_HIT lpn=%"PRIu64" ppn=%"PRIu64"\n",
                lpn,
                entry->ppn
            );
        ...
        dftl_verify(
            "READ_MAP lpn=%"PRIu64" ppn=%"PRIu64"\n",
            lpn,
            ppa2pgidx(ssd, &ppa)
        );
...
static uint64_t ssd_write(struct ssd *ssd, NvmeRequest *req)
{
    ...
    for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
        ...
        if (mapped_ppa(&ppa)) {                              
            dftl_verify(
                "INVALIDATE_OLD lpn=%"PRIu64" old_ppn=%"PRIu64"\n",
                lpn,
                ppa2pgidx(ssd, &ppa)
            );
            /* update old page information first */
        ...
        dftl_verify(
            "WRITE_MAP lpn=%"PRIu64" ppn=%"PRIu64"\n",
            lpn,
            ppa2pgidx(ssd, &ppa)
        );
    ...
    // #ifdef DFTL_FORCE_GC_TEST
    // {
    //     static bool forced_gc_done = false;

    //     if (!forced_gc_done && ssd->lm.victim_line_cnt > 0) {
    //         dftl_verify(
    //             "FORCE_GC_TRIGGER victim=%d free=%d\n",
