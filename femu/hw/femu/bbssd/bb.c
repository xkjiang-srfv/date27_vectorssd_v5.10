...
static uint16_t bb_nvme_nodeid_rw(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd,
                           NvmeRequest *req)
{
    return nvme_nodeid_rw(n, ns, cmd, req);
}

static uint16_t bb_nvme_aggregated_read(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    return nvme_aggregated_read(n, ns, cmd, req);
}
                          ...
                          NvmeRequest *req)
{
    ...
    /*-specific NVMe IO commands*/
    case NVME_CMD_NODEID_WRITE:
    case NVME_CMD_NODEID_READ:
        return bb_nvme_nodeid_rw(n,ns,cmd,req);
    case NVME_CMD_AGGRE_READ:
        return bb_nvme_aggregated_read(n, ns, cmd, req);
    default:
