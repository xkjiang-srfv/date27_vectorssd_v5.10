#include "./nvme.h"
#include "./bbssd/raw_region.h"
#include <inttypes.h>
#include "./bbssd/aggregation.h"
...
uint16_t nvme_rw(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    ...
    // femu_log("[NVME_RW] opcode=0x%x is_write=%d slba=%lu nlb=%u\n", rw->opcode, req->is_write, slba, nlb);
...
static inline uint64_t ns_total_bytes(NvmeNamespace *ns)
{
     
    const uint8_t lba_index  = NVME_ID_NS_FLBAS_INDEX(ns->id_ns.flbas);
    const uint8_t data_shift = ns->id_ns.lbaf[lba_index].lbads;
    uint64_t nsze = le64_to_cpu(ns->id_ns.nsze);
    return nsze << data_shift;
}

uint16_t nvme_aggregated_read(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    NvmeAggreReadCmd *acmd = (NvmeAggreReadCmd *)cmd;

    uint64_t mptr = le64_to_cpu(acmd->mptr);
    uint64_t prp1 = le64_to_cpu(acmd->prp1);
    uint64_t prp2 = le64_to_cpu(acmd->prp2);
    uint32_t result_nblks = le32_to_cpu(acmd->result_nlb);

    // femu_log("[AGGR_READ] raw mptr=0x%lx prp1=0x%lx prp2=0x%lx result_nlb=%u opcode=0x%x\n",
        //  mptr, prp1, prp2, result_nblks, req->cmd.opcode);
    ...
    req->is_write = 0;
    req->status = NVME_SUCCESS;
    req->nlb = result_nblks;

    int poller = req->sq->sqid;                      
    if (poller <= 0 || poller > n->nr_pollers) {
        poller = 1;
    }

    int rc = femu_ring_enqueue(n->to_raw[poller], (void *)&req, 1);
    if (rc != 1) {
        femu_err("AGGR_READ enqueue to_raw failed, poller=%d\n", poller);
...
uint16_t nvme_nodeid_rw(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd, NvmeRequest *req)
{
    NvmeNodeCmd *node = (NvmeNodeCmd *)cmd;
    uint64_t data_offset = 0;
    uint64_t data_size = 0;

    const bool is_write = (node->opcode == NVME_CMD_NODEID_WRITE);
    uint64_t prp1   = le64_to_cpu(node->prp1);               
    uint64_t prp2   = le64_to_cpu(node->prp2);               
    uint64_t nodeid = le64_to_cpu(node->nodeid);             
    uint32_t nblks  = (uint32_t)le16_to_cpu(node->nlb);      
        

     
    if (nblks == 0) {
        nvme_set_error_page(n, req->sq->sqid, cmd->cid, NVME_INVALID_FIELD, offsetof(NvmeNodeCmd, nlb), 0, ns->id);
        return NVME_INVALID_FIELD | NVME_DNR;
    }

     
    if (nodeid == 0) {
        nblks = 1;                         // metadata page
        data_offset = 0;
        data_size = FLASH_PAGE_BYTES;       
    } else {
        data_offset = lftl_node_byte_offset(nodeid);
        data_size = LEARNED_NODE_BYTES;     
    }
    
    uint64_t cap = ns_total_bytes(ns);
    if (data_offset + data_size > cap) {
        nvme_set_error_page(n, req->sq->sqid, cmd->cid, NVME_LBA_RANGE, offsetof(NvmeNodeCmd, nodeid), 0, ns->id);
        return NVME_LBA_RANGE | NVME_DNR;
    }

    // femu_log("%s nodeid=%lu nblks=%u offset=%lu size=%lu\n", is_write ? "[WRITE]" : "[READ]",  nodeid, nblks, data_offset, data_size);    
     
    if (nvme_map_prp(&req->qsg, &req->iov, prp1, prp2, data_size, n)) {
        nvme_set_error_page(n, req->sq->sqid, cmd->cid, NVME_INVALID_FIELD, offsetof(NvmeNodeCmd, prp1), 0, ns->id);
        return NVME_INVALID_FIELD | NVME_DNR;
    }

    req->is_write = is_write ? 1 : 0;
    req->status   = NVME_SUCCESS;
    req->nlb  = nblks;
    req->slba = nodeid;
    int ret = backend_rw(n->mbe, &req->qsg, &data_offset, req->is_write);
    return ret ? NVME_DNR : NVME_SUCCESS;
}
